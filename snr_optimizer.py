...
from radar_simulator.signal_processing.signal_processing import pulse_compression, produce_rd_map
...
class OptimizeSNR(nn.Module):
    ...
    def __init__(self, seq_len: int, kernel: Tensor, *args, **kwargs):
        ...
        self.weights_angle = nn.Parameter(0*torch.arange(seq_len, dtype=torch.float32).unsqueeze(-1))
    ...
    def forward(self, x: Tensor):
        ...
        x = x.shape[0] * torch.softmax(self.weights_mag, dim=0) * x * torch.exp(-1j * phase)
        return x
...
def snr(rd_map: Tensor)->Tensor:
    ...
    peak_idx = circular_weigthed_mean(rd_map_energy[:, max_range_index])
    peak = rd_map_energy[torch.floor(peak_idx).to(torch.int), max_range_index]
    # peak = rd_map_energy[max_doppler_index, max_range_index]
    ...
    return 10*torch.log10(peak.sum()) - 10*torch.log10(avg_noise) #- 10*torch.log10(global_noise) - 10 * torch.log10(doppler_noise)
...
model = OptimizeSNR(seq_len=dataset.radar.n_pulses, kernel=dataset.kernel).to(device=device)
...
after_mf = pulse_compression(iq.cpu().to(torch.complex128), dataset.radar.pulse_bb.to(torch.complex128))
...
fig, ax = plt.subplots()
ax.plot(rd_map_db[:, max_range_index]-rd_map_db[:, max_range_index].max(), label='Classic')
ax.plot(rd_map_fixed_db[:, max_range_index]-rd_map_fixed_db[:, max_range_index].max(), label='Pred')
ax.axvline(x=doppler_index*nfft/dataset.radar.n_pulses, color='r', label='True Doppler')
...
columns = ['Range', 'Doppler', 'Doppler Opt', 'Doppler Classic', 'SNR Opt', 'SNR Classic']
...
for index, (iq, (range_index, doppler_index)) in tqdm(enumerate(dataset)):
    ...
    noise_opt = (best_rd_map[:, max_range_index].sum() - max_opt)/(best_rd_map.shape[0]-1)
    snr_opt = 10 * torch.log10(max_opt/noise_opt)

    df.loc[index, 'Doppler Opt'] = max_doppler_index.item()
    df.loc[index, 'SNR Opt'] = snr_opt.item()
    ...
    noise_classic = (rd_map_classic[:, max_range_index].sum() - max_classic)/(rd_map_classic.shape[0]-1)
    snr_classic = 10 * torch.log10(max_classic/noise_classic)

    df.loc[index, 'Doppler Classic'] = circular_weigthed_mean(rd_map_classic[:, max_range_index].to(device=device)).item()
...
plt.hist(df_diff_classic, bins=25, alpha=0.75, label=f'Classic - STD = {df_diff_classic.std(): .2f}')
plt.hist(df_diff_opt, bins=25, alpha=0.75, label=f'Opt - STD = {df_diff_opt.std(): .2f}')
plt.legend()
